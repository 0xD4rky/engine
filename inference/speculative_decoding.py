...
def speculative_decoding_with_kv_cache(
    ...
    while generated_ids.shape[1] < max_new_tokens and not finished.all():
        ...
        # Clone the draft cache so speculative decoding does not mutate the live cache
        ...
        for _ in range(step_budget):
            ...
            speculative_tokens = next_token.unsqueeze(1)
        ...
        alive = active.clone()
        target_logits_step = target_logits_next
        draft_logits_live = draft_logits_next

        for step in range(block_len):
            if not alive.any() or generated_ids.shape[1] >= max_new_tokens:
                break

            target_probs = processor(target_logits_step)
            ...
            accept_rand = torch.rand_like(acceptance)
            accept_mask = (accept_rand <= acceptance) & alive
            reject_mask = alive & ~accept_mask
            ...
            if mask_column.any():
                ...
                draft_cache, draft_logits_live = _forward_step(
                    draft_model,
                    token_column_unsq,
                    current_mask,
                    draft_cache,
        ...
        if alive.any():
            target_probs = processor(target_logits_step)
            next_token = processor.sample(target_probs).squeeze(-1)
        ...
        draft_logits_next = draft_logits_live
